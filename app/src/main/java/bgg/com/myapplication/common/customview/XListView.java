...
public class XListView extends ListView implements OnScrollListener {
	...
	protected XFooterView getFooterView(Context context) {
	...
	protected void initWithContext(Context context) {
		...
		mHeader = getHeaderView(context);

		mHeaderContent = mHeader.getHeaderContainer();
		addHeaderView(mHeader, null, false);

		// init footer view

		mFooterView = getFooterView(context);
	...
	public boolean getListPullState() {
		return mIsPullDown;
	}
	...
	public void resetFooterView() {
		mFooterView.setState(XFooterView.STATE_FIRST);
		mEnablePullLoad = false;
		mEnableAutoLoad = false;
	}

	public void setXListViewListEnd(boolean isEnd) {
		mEnablePullLoad = !isEnd;
		mEnableAutoLoad = !isEnd;
		if (isEnd) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		switch (ev.getAction()) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				mLastY = ev.getRawY();
				mIsPullDown = deltaY > 0 ? true : false;

				if (getFirstVisiblePosition() == 0
						&& (mHeader.getVisibleHeight() > 0 || deltaY > 0)) {
					// the first item is showing, header has shown or pull down.
					if (mEnablePullRefresh == true) {
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		...
		if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
			// 因为当内容很少的时候，下拉刷新的时候,触发了该 onScrollStateChanged(), 固需要一个标志位
			// mIsPullDown来区分是否需要load下一批
	...
	private void loadMore() {
		if (mEnablePullLoad && null != mListener && !mPullLoading) {
			mPullLoading = true;
